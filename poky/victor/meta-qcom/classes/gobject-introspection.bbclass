...
EXTRA_OECONF:prepend_class-target = "${@bb.utils.contains('GI_DATA_ENABLED', 'True', '--enable-introspection', '--disable-introspection', d)} "

# When building native recipes, disable introspection, as it is not necessary,
# pulls in additional dependencies, and makes build times longer
EXTRA_OECONF:prepend_class-native = "--disable-introspection "
EXTRA_OECONF:prepend_class-nativesdk = "--disable-introspection "

UNKNOWN_CONFIGURE_OPT_IGNORE:append = " --enable-introspection --disable-introspection"

# Generating introspection data depends on a combination of native and target
# introspection tools, and qemu to run the target tools.
# DEPENDS:append_class-target = " gobject-introspection gobject-introspection-native qemu-native prelink-native"

# Even though introspection is disabled on -native, gobject-introspection package is still
# needed for m4 macros.
DEPENDS:append_class-native = " gobject-introspection-native"
DEPENDS:append_class-nativesdk = " gobject-introspection-native"

# This is used by introspection tools to find .gir includes
export XDG_DATA_DIRS = "${STAGING_DATADIR}"

do_configure:prepend_class-target () {
...
FILES:${PN}-dev:append = " ${datadir}/gir-*/*.gir"
