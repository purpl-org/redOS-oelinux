...
#LIBGCC = "${@bb.utils.contains('GCCVERSION', '5.2%', '${STAGING_LIBDIR}/${TARGET_SYS}/5.2.0/libgcc.a', '${STAGING_LIBDIR}/${TARGET_SYS}/4.9.3/libgcc.a', d)}"
LIBGCC = "${STAGING_LIBDIR}/${TARGET_SYS}/7.3.0/libgcc.a"
...
EXTRA_OEMAKE:append = " VERIFIED_BOOT=0 DEFAULT_UNLOCK=true EMMC_BOOT=${emmc_bootloader} APPEND_CMDLINE=${emmc_bootloader}"

EXTRA_OEMAKE:append = " ${@bb.utils.contains('DISTRO_FEATURES', 'systemd', 'USE_LE_SYSTEMD=true', '', d)}"

EXTRA_OEMAKE:append = " ${@bb.utils.contains('DISTRO_FEATURES', 'vble', 'VERIFIED_BOOT_LE=1', '', d)}"

EXTRA_OEMAKE:append = " ${@oe.utils.conditional('USER_BUILD', '1', 'TARGET_BUILD_VARIANT=user', '', d)}"
...
#enable hardfloat
EXTRA_OEMAKE:append = " ${@oe.utils.conditional('ARM_FLOAT_ABI', 'hard', 'ENABLE_HARD_FPU=1', '', d)}"
