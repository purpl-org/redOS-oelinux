...
SRC_URI:append_qemuall = " file://wired.config \
                           file://wired-setup \
...
do_install() {
    ...
    if test -e ${S}/wired.config &&
       test -e ${S}/wired-setup &&
       test -e ${S}/wired-connection.service; then
        install -d ${D}${localstatedir}/lib/connman
        install -m 0644 ${S}/wired.config ${D}${localstatedir}/lib/connman
        install -d ${D}${datadir}/connman
        install -m 0755 ${S}/wired-setup ${D}${datadir}/connman
        install -d ${D}${systemd_system_unitdir}
        install -m 0644 ${S}/wired-connection.service ${D}${systemd_system_unitdir}
        sed -i -e 's|@SCRIPTDIR@|${datadir}/connman|g' ${D}${systemd_system_unitdir}/wired-connection.service
    fi
}

#SYSTEMD_SERVICE${PN}_qemuall = "wired-connection.service"
