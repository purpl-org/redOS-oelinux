...
docker run -it --rm \
    ...
    vic-yocto-builder-7 bash -c "cd $(pwd)/poky && source build/conf/set_bb_env.sh && MACHINE=apq8009-robot VARIANT=perf DISTRO=msm-${VARI} PRODUCT=robot bitbake -c cleanall $@"
